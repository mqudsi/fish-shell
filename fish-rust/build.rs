...
fn detect_features(target: Target) {
    for (feature, handler) in [
        ...
        (
            "",
            &(|_: &Target| Ok(false)) as &dyn Fn(_) -> miette::Result<bool>,
        ),
        ("bsd", &detect_bsd),
...
fn detect_bsd(_: &Target) -> miette::Result<bool> {
...
/// support.
fn have_gettext(target: &Target) -> miette::Result<bool> {
    ...
    if let Some(with_gettext) = std::env::var_os("CMAKE_WITH_GETTEXT") {
        if with_gettext.eq_ignore_ascii_case("0") {
            rsconf::warn!("gettext disabled in CMake");
            return Ok(false);
        }
    }
    ...
    match found {
        0 => {
            rsconf::warn!("gettext not found");
            Ok(false)
        }
        1 => miette::bail!("gettext found but cannot be used without {}", symbols[1]),
        _ => {
