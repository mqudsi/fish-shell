...
fn update_fish_color_support(vars: &EnvStack) {
    let max_colors = curses::term().and_then(|term| term.get(curses::MAX_COLORS));
...
fn initialize_curses_using_fallbacks(vars: &EnvStack) {
    ...
    for term in FALLBACKS {
        ...
        let success = curses::setup(Some(&term_cstr), libc::STDOUT_FILENO).is_some();
...
fn apply_term_hacks(vars: &EnvStack) {
    ...
    // Overriding term capabilities requires that we've successfully initialized the curses
    // subsystem.
            ...
            if let Some(term_val) = vars.get(L!("TERM")).map(|v| v.as_string()) {
                if term_val == "xterm-256color" {
                    ...
                    if term.get(curses::ENTER_ITALICS_MODE).is_none() {
                        term.set(curses::ENTER_ITALICS_MODE, Some(SITM_ESC));
                    }
                    if term.get(curses::EXIT_ITALICS_MODE).is_none() {
                        term.set(curses::EXIT_ITALICS_MODE, Some(RITM_ESC));
                    }
                    if term.get(curses::ENTER_DIM_MODE).is_none() {
                        term.set(curses::ENTER_DIM_MODE, Some(DIM_ESC));
                    }
                }
            }
        }
...
fn init_curses(vars: &EnvStack) {
    ...
    if curses::setup(None, libc::STDOUT_FILENO).is_none() {
        if is_interactive_session() {
