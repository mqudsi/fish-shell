...
use crate::ffi::Repin;
use crate::ffi::{
    builtin_exists, colorize_shell, function_exists, function_get_annotated_definition,
    function_get_copy_definition_file, function_get_copy_definition_lineno,
    function_get_definition_file, function_get_definition_lineno, function_is_copy,
    path_get_paths_from_parser,
};
        ...
        if !opts.force_path && !opts.no_functions && function_exists(&arg.to_ffi(), parser.pin()) {
            ...
            if !opts.query && !opts.get_type {
                ...
                if (*path).is_empty() {
                    ...
                    let lineno: i32 =
                        i32::from(function_get_definition_lineno(&arg.to_ffi(), parser.pin()));
                    comment.push_utfstr(&wgettext_fmt!("Defined in %ls @ line %d", path, lineno));
                }
                ...
                if function_is_copy(&arg.to_ffi(), parser.pin()) {
                    ...
                    if (*path).is_empty() {
                        ...
                        let lineno: i32 = i32::from(function_get_copy_definition_lineno(
                            &arg.to_ffi(),
                            parser.pin(),
                        ));
                ...
                if opts.path {
                    if function_is_copy(&arg.to_ffi(), parser.pin()) {
                        let path = function_get_copy_definition_file(&arg.to_ffi(), parser.pin())
                            .from_ffi();
                    ...
                    def.push_utfstr(&sprintf!(
                        "# %ls\n%ls",
                        comment,
                        function_get_annotated_definition(&arg.to_ffi(), parser.pin()).from_ffi()
                    ));
