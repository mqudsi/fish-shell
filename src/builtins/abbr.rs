...
fn abbr_show(streams: &mut IoStreams) -> Result<Option<()>, NonZeroU8> {
...
fn abbr_list(opts: &Options, streams: &mut IoStreams) -> Result<Option<()>, NonZeroU8> {
...
fn abbr_rename(opts: &Options, streams: &mut IoStreams) -> Result<Option<()>, NonZeroU8> {
    ...
    abbrs::with_abbrs_mut(|abbrs| -> Result<Option<()>, NonZeroU8> {
...
fn abbr_query(opts: &Options) -> Result<Option<()>, NonZeroU8> {
...
fn abbr_add(opts: &Options, streams: &mut IoStreams) -> Result<Option<()>, NonZeroU8> {
...
fn abbr_erase(opts: &Options, parser: &Parser) -> Result<Option<()>, NonZeroU8> {
    if opts.args.is_empty() {
    ...
    abbrs::with_abbrs_mut(|abbrs| -> Result<Option<()>, NonZeroU8> {
        ...
        for arg in &opts.args {
            if !abbrs.erase(arg) {
                result = EnvStackSetResult::ENV_NOT_FOUND.into();
            }
